...
abstract class Email {
    public static function enviarConfirmacion($token, $correo) {
        ...
        $apiKey = $_ENV["EMAIL_APIKEY"];

        try {
            $mail = Mailgun::create($apiKey);
            $email = $mail->messages()->send($domain, array(
                'from' => 'OdonSecure <'.$_ENV["EMAIL_USERNAME"].'>',
                'to' => $correo,
                'subject' => 'Confirmación Cuenta',
                'text' => 'En el siguiente enlace &raquo; <a href="'. $_ENV["APP_URL"] .'/confirm?token='.$token.'">Confirmar Cuenta</a>, podras confirmar tu cuenta en OdonSecure para obtener todos los beneficios disponibles.',
                'html' => $html
            ));
        }
        catch(Exception $e) {
    ...
    public static function enviarReestablecer($token, $correo) {
        ...
        try {
            ...
            $email = $mail->messages()->send($domain, array(
                'from' => 'OdonSecure <'.$_ENV["EMAIL_USERNAME"].'>',
                'to' => $correo,
                'subject' => 'Reestablecer Cuenta',
                'text' => 'En el siguiente enlace &raquo; <a href="'. $_ENV["APP_URL"] .'/change?token='.$token.'">Reestablecer Contrase&ntilde;a</a>, podras reestablecer tu contrase&ntilde;a en OdonSecure para obtener todos los beneficios disponibles.',
                'html' => $html
            ));
        }
        catch(Exception $e) {
